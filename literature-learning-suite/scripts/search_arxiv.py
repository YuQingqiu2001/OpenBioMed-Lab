...
def search(query=None, author=None, category=None, ids=None, max_results=5, sort="relevance"):
    ...
    # The HTTP endpoint reliably follows arXiv's redirect and also works with
    # proxy settings inherited from http_proxy/https_proxy.
    url = "http://export.arxiv.org/api/query?" + "&".join(f"{k}={v}" for k, v in params.items())
